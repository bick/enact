...
import {Button} from '@enact/ui/components/button';
import {cn} from "@enact/utils/styles";
    ...
    return (
        <nav
            className={cn(
                ...
                isHomepage ? 'fixed top-0 left-0 right-0' : 'sticky top-0 bg-black',
                isMenuOpen || activeMegaMenu ? 'bg-black' : '',
                (isHomepage && hasScrolled) || !isHomepage ? 'bg-black max-h-fit' : '',
                isVisible ? 'translate-y-0' : '-translate-y-full',
            )}
        >
            <div className="mx-auto flex items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
                <div className="flex items-center">
                    <Link href="/apps/web/public" className="flex items-center">
                        <Image className="w-auto block mr-4" src="/assets/apollo.png" width={36} height={36}
                               alt="HeadlessWP"/>
                    ...
                    <div className="flex items-center space-x-2">
                        {menuItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                ref={item.name === 'Product' ? productButtonRef : undefined}
                                className={cn(
            ...
            <div
                ...
                className={cn(
                    ...
                    activeMegaMenu === 'Product' ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0 pointer-events-none',
                )}
